...
/// aliases `blob`. Rejects a blob whose type or curve string is wrong, or whose point is not a
/// 65-byte 0x04-prefixed point. Trailing bytes after the point are ignored, so a key_id need only
/// begin with a well-formed blob.
pub fn pointFromPubBlob(blob: []const u8) ![]const u8 {
