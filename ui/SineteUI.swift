...
struct ReadyView: View {
    ...
    private func uninstall() {
        // Pass 3 adds the key-removal prompt + warnings; here it tears down the
        // install (login item, link, PATH) and keeps the keys.
        busy = true
        DispatchQueue.global().async {
            var failure: String?
            do { try Backend.run(["uninstall"]) } catch { failure = error.localizedDescription }
            DispatchQueue.main.async {
                busy = false
...
struct SetupView: View {
    ...
    private var stepSSH: some View {
        VStack(spacing: 12) {
            Text("Set up the key for SSH and Git signing?")
                .foregroundStyle(.secondary)
            HStack {
                Button("Skip") { onDone() }
    ...
    private func install() {
        ...
        DispatchQueue.global().async {
            ...
            if let planOut = try? Backend.run(["install", "--plan"]),
               ...
               plan.linkConflicts {
                let go = DispatchQueue.main.sync { () -> Bool in
                    let alert = NSAlert()
                    alert.messageText = "Replace existing link?"
                    alert.informativeText = "\(plan.linkPath) already points elsewhere. Replace it so it points at sinete?"
            ...
            var args = ["install"]
            if replace { args.append("--replace-link") }
            var failure: String?
