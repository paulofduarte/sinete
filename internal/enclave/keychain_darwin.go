...
// epochGet reads the epoch from its keychain item; (0, false, nil) when absent.
...
// epochSet stores an arbitrary epoch value (8-byte big-endian).
func epochSet(v uint64) error {
	var b [8]byte
	binary.BigEndian.PutUint64(b[:], v)
	return keychainItemSet(EpochService, EpochAccount, b[:])
}

// epochDelete removes the epoch item (absence tolerated).
...
// epochIncrement advances the epoch by one (read+1+store). Non-atomic — the
