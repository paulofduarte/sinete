...
// macOS Touch ID presence gesture for the Authorizer seam. Ported from the Go sinete
// presence_darwin.m: evaluate the device-owner authentication policy (Touch ID with a passcode
// fallback). evaluatePolicy is asynchronous, so we pump the run loop on the calling thread (which
// must be the main thread) until the reply arrives, letting the system present the prompt. The
// libxev agent loop is single-threaded on the main thread, so this is invoked there directly.
...
int sinete_authenticate(const char *reason, char **err) {
    @autoreleasepool {
        ...
        // stringWithUTF8String: returns nil for a NULL or invalid-UTF-8 reason, and passing nil to
        // localizedReason: would crash; fall back to a generic reason.
        ...
        // Pump the run loop so the system can present the prompt, until the reply signals. The
        // semaphore both ends the loop and creates a happens-before edge with the reply block, so
        // reading ok/errmsg below is race-free.
